...
/* NASA API listener & function */
document.querySelector('.getNasaData').addEventListener('click', getFetch)

function getFetch(){
  const choice = document.querySelector('.dateInput').value
  console.log(choice)

  fetch(`https://api.nasa.gov/planetary/apod?api_key=ZwuQVjgy10jTIWss56oqG2XqpmsGAbmZTlJZlvHi&date=${choice}`)
  .then(res => res.json())
  .then(data => {
    console.log(data)
    if(data.media_type === 'image'){
      ...
      document.querySelector('.nasaPic').src = ''
